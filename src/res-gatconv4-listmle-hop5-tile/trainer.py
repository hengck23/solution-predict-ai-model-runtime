...
def run_train(cfg):
	...
	train_dataset = TileDataset(train_data, subsample_param = dict(size=cfg.train_subsample_size, is_random=True, is_droplast=True))
	valid_dataset = TileDataset(valid_data, subsample_param = dict(size=cfg.valid_subsample_size, is_random=False, is_droplast=False))
	...
	train_loss = MyMeter([0, 0, 0,], min(20,num_train_batch))#window must be less than num_train_batch
	valid_loss = [0, 0,  0,]
	...
	def message_header():
		...
		text += f'rate      iter       epoch  |  kendall   opa    slowdown  |  listMLE  opa slowdown | time         \n'
		text += f'--------------------------------------------------------------------------------------------------\n'
	...
	def do_valid():
		...
		# ----
		write_pickle_to_file(f'{cfg.fold_dir}/result.pickle',result)
		N = len(result)
		kendall_tau  = []
		slowndown_acc   = []
		for i in range(N):
			...
			top5 = np.argsort(predict)[:5]
			slowndown = 2 - truth[top5].min() / truth.min()
			slowndown_acc.append(slowndown)
		...
		for t, batch in enumerate(train_loader):
			...
			for b in range(B):
				...
				with torch.cuda.amp.autocast(enabled=True):
					...
					topk_acc = output[f'topk_acc'].mean()/B

					rank_loss_item += rank_loss.item()
					opa_acc_item += opa_acc.item()
					topk_acc_item += topk_acc.item()
			...
			batch_loss = [rank_loss_item,opa_acc_item,topk_acc_item]
